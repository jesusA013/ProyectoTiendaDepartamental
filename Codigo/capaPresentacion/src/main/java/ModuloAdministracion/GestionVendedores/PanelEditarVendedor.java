...
public class PanelEditarVendedor extends javax.swing.JPanel {
    ...
    /**
     * Creates new form PanelEditarVendedor
     */
    public PanelEditarVendedor(JPanel panelCambiante,IVendedorBO vendedorBO,VendedorDTO vendedorDTO) {
        initComponents();
        ...
        this.cargarDatosVendedor(vendedorDTO);
    }
    ...
    private void editarVendedor() {
        try {
            ...
            // Armar el vendedor
            vendedorDTO.setCurp(txtCURP.getText().trim());
            vendedorDTO.setNombreCompleto(nombre);
            vendedorDTO.setDomicilio(domicilio);
            vendedorDTO.setDatosFiscales(datosFiscales);
            vendedorDTO.setSeguro(seguro);
            vendedorDTO.setFechaNacimiento(fechaNacimiento);
            vendedorDTO.setEstadoCivil(comboEstadoCivil.getSelectedItem().toString());
            vendedorDTO.setActivo(chkActivo.isSelected());

            // Insertar
            VendedorDTO vendedorInsertado = vendedorBO.actualizarVendedor(vendedorDTO);

            JOptionPane.showMessageDialog(this, "Vendedor editado con éxito.\nID: " + vendedorInsertado.getId());
            this.volver();

        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, "Error al editado vendedor:\n" + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            e.printStackTrace();
        }
    }
    private void cargarDatosVendedor(VendedorDTO vendedor) {
        if (vendedor == null) return;

        txtCURP.setText(vendedor.getCurp());
        txtNombres.setText(vendedor.getNombreCompleto().getNombres());
        txtApellidoPaterno.setText(vendedor.getNombreCompleto().getApellidoPaterno());
        txtApellidoMaterno.setText(vendedor.getNombreCompleto().getApellidoMaterno());
        comboEstadoCivil.setSelectedItem(vendedor.getEstadoCivil());

        // Datos fiscales
        DatosFiscalesDTO df = vendedor.getDatosFiscales();
        if (df != null) {
            txtRFC.setText(df.getRfc());
            txtCorreoFiscal.setText(df.getCorreo());
            txtCalleFiscal.setText(df.getDomicilioFiscal().getCalle());
            txtMunicipioFiscal.setText(df.getDomicilioFiscal().getDelegacionMunicipio());
            txtCiudadFiscal.setText(df.getDomicilioFiscal().getCiudadLocalidad());
            txtCPFiscal.setText(df.getDomicilioFiscal().getCodigoPostal());
        }

        // Domicilio
        DomicilioDTO domicilio = vendedor.getDomicilio();
        if (domicilio != null) {
            txtCalleDom.setText(domicilio.getCalle());
            txtMunicipioDom.setText(domicilio.getDelegacionMunicipio());
            txtCiudadDom.setText(domicilio.getCiudadLocalidad());
            txtCPDom.setText(domicilio.getCodigoPostal());
        }

        // Seguro
        SeguroDTO seguro = vendedor.getSeguro();
        if (seguro != null) {
            txtNumSeguro.setText(seguro.getNumeroSeguro());
            comboTipoSeguro.setSelectedItem(seguro.getTipo());
        }
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        txtFechaNacimiento.setText(sdf.format(vendedor.getFechaNacimiento()));
    ...
    private void initComponents() {
        ...
        gestionProductosBTN1.setText("Editar Vendedor");
